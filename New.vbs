...
Function KillExe(exename,server)
	...
	Dim oShell : Set oShell = CreateObject("WScript.Shell")
	' Include /t tree kill
	Dim iRet : iRet = oShell.Run("taskkill /S " & server & " /f /t /IM " & exename, 1, True)
	If iRet = 0 Then
		WriteMsg "Kill " & server & " " & exename & " Success. Code: " & iRet
	Else
		If iRet = 255 Then	
			'do not pass back error for 255 code (testing verifies that .exe has stopped e.g. in ECM jboss example)
			WriteMsg "Kill " & server & " " & exename & " Success with Code: " & iRet
			iRet = 0
		Else
	...
	KillExe = iRet
	If Err.Number <> 0 then
		' Code 255 still results in service being stopped
...
Function RunLogCleanup()
	...
	Else
		'Avoid errors being output from this non-essential maintenance cleanup to allow success to be reported:
...
Function Main
	...
	'Objective Application Server - do not stop service - may not respond
	...
	WriteMsg "Kill Objective Application Server jboss process"
	iError = KillExe("jbosssvc.exe", OBJECMSVR )
	If iError <> 0 Then
		bSuccess = False
	End If
...
Do
	...
	'No need to check for Objective Application Server stopped, as already killed JBoss above
